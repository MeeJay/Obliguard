...
//
//  1. Old exe downloads  obliview-agent.msi  to %TEMP%
//  2. Old exe writes %TEMP%\obliview-msi-update.bat  and launches it detached
//  3. Old exe calls restartWithNewBinary() → os.Exit(0)   (service stops)
//  4. Batch waits 2 s, then runs:
//       msiexec /i obliview-agent.msi /quiet /norestart SERVERURL=... APIKEY=...
//     MSI stop the service (already stopped), overwrites obliview-agent.exe,
